...
// 评论回复/头像缓存
function veryer_comment($comment, $args, $depth) {
...
<li <?php comment_class(); ?> id="comment-<?php comment_ID() ?>">
 ...
 ?>
         </div><strong><?php comment_author_link() ?></strong>:<?php get_author_class($comment->comment_author_email,$comment->comment_author_url)?>  <?php edit_comment_link('编辑','&nbsp;&nbsp;',''); ?></div>
...
global $wpdb;
$adminEmail = 'tuo@zycao.com';
$author_count = count($wpdb->get_results(
"SELECT comment_ID as author_count FROM $wpdb->comments WHERE comment_author_email = '$comment_author_email' "));
if($comment_author_email ==$adminEmail)
echo '<a class="vip7" ></a><a class="vip"  title="我是博主"></a>';
...
function wp_today(){
 ...
 if ( $h_post ){
  $result = "<ul><h3>她们和我一起过生日:</h3>".$h_post."</ul>";
 }
 return $result;
 wp_reset_query();
}
...
//增强搜索相关性 （先按照日志的标题的相关性排序，然后按照日志的内容，最后才是日的志修改时间和 ID）
